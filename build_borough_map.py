"""
Fetches NYC borough boundaries (NYC Open Data, dataset gthc-hcne) and bakes them
into a lightweight static SVG-path asset for the borough heat map on the site.
Geometry is static (boroughs don't move) so this only needs to be rerun if the
projection/simplification needs tuning; live violation counts still come from
/api/rodent-summary at page load.
"""
import json
import math
import os
...
BASE_URL = "https://data.cityofnewyork.us/resource/gthc-hcne.geojson"
REQUEST_TIMEOUT = 60
...
def main():
    ...
    for feature in fc["features"]:
        boro = feature["properties"]["boroname"].upper()
        ...
        print(f"  {boro}: {n_points} points after simplification")
    ...
    boroughs = {}
    for boro, geom in projected.items():
    ...
    out_path = Path(__file__).resolve().parent / "public" / "nyc-boroughs.json"
